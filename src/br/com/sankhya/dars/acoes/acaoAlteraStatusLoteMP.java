...
public class acaoAlteraStatusLoteMP implements AcaoRotinaJava {
    @Override
    public void doAction(ContextoAcao contextoAcao) throws Exception {
        ...
        for (Registro linha : linhas) {
            ...
            Collection<DynamicVO> ite = iteDAO.find("NUNOTA=? AND (USOPROD = 'E' OR USOPROD = 'M')", linha.getCampo("NUNOTA"));
            for(DynamicVO iteVO :ite){
                ...
                try {
                    acessoBanco.update("UPDATE TGFITE SET STATUSLOTE = 'P' WHERE NUNOTA = ? AND CODPROD = ? AND (USOPROD = 'E' OR USOPROD = 'M')",nunota,codprod);
